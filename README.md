...
if (dati.type === "/tuio/2Dobj") {
  fiduciali[dati.session_id] = { id: dati.class_id, x: dati.x*width, y: dati.y*height, angolo: dati.angle};
...
function draw() {
  ...
  for (let sid in fiduciali) {
    ...
    textSize(22);
    // Mostriamo il numero stampato (id)!
    text(f.id, f.x, f.y);
...
function setup() {
  ...
  socket.onmessage = function(event) {
    ...
    // 1. DATA HANDLING
    if (dati.type === "/tuio/2Dobj") {
      // Store object data: id, position, and rotation angle
      ...
      // Store cursor/touch data
      cursori[dati.session_id] = { 
        x: dati.x * width, 
        y: dati.y * height 
      };
    }
    ...
    // 2. CLEANUP (ALIVE)
      ...
      // Remove objects that are no longer detected by the camera
      let targetObj = (dati.source === "/tuio/2Dobj") ? fiduciali : cursori;
      for (let sid in targetObj) {
        if (!dati.session_ids.includes(Number(sid))) {
          delete targetObj[sid];
...
function draw() {
  ...
  text(logStato, 20, 20);
  fill(0, 255, 200);
  text("Objects detected on table: " + Object.keys(fiduciali).length, 20, 50);
  ...
  for (let sid in fiduciali) {
    ...
    // Draw the fiducial ID inside the circle
    fill(255);
    noStroke();
    textAlign(CENTER, CENTER);
    textSize(22);
